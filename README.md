...
17. In the Settings menu, below the button `Copy to Clipboard` for the Recovery Phrase there should be a button to generate a QRCode so another device's camera can read it. IMPORTANT: the QRCode generation should be ONLY local (no data sent to a server) and should NEVER store any of this data locally. For the second device to read the QRCode, there should be a button next to `Restore` (that shows only after user clicked on `Restore from Phrase`) that allows user to use the camera to read the QRCode to populate the Recovery Phrase. The same option should be available when setting up a new database upon starting from fresh (i.e. device with no local database)
18. The app should support multiple Evolu databases to be used at the same time. They should each be in a separate tab, the tabs should show at the top after the `Numpad` title. For each new database it should start with the same splash screen as if the app was opened for the first time. Obviously each database should have its own `Settings` menu since each one will have its own Evolu `mnemonics`. User should be able to close and open new databases just like we can open/close new tabs in a browser
19. Use `dnd-kit` (https://docs.dndkit.com/) to implement drag-and-drop to be able to move items (notes and calculator-notes) in and out of folders. We should also be able to reorder tabs at the top that represent different databases. On desktop the item animation should be at the same position as the mouse pointer while drag-and-dropping an item. On the mobile it should also work seamless, with the position of the touch being the place where the action is taken and where the animated item being dragged should follow. IMPORTANT to read the documentation for the `dnd-kit` and their examples to follow the best-practices on how to implement this library